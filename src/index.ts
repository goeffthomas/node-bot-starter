...
const searchForNodeBotStarter = async () => {
  // Navigate to Google
  await driver.get(PAGE_URL);

  // Find the search bar
  const searchBar = await safelyGetElementByLocator(
    driver,
    By.css('[aria-label="Search"]'),
  );
  ...
  const searchBtns = await driver.findElements(
    By.css('input[value="Google Search"]'),
  );
  ...
  const clickableLinks = await Promise.all(
